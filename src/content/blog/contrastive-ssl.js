export default {
  ...
  lead:
    'A representation is a geometry you can reuse, not a class name. Contrastive methods grow it by pushing negatives apart. Self-distillation methods match a student to a teacher without a negative batch. Collapse is the shared failure. DINO lives in the second family, not in InfoNCE with extra crops.',
  concept: {
  ...
  sections: [
    {
      ...
      body: [
        ...
        'Contrastive methods pull two views together and push others apart with InfoNCE — SimCLR on the batch, MoCo on a queue. BYOL and SimSiam drop negatives and predict a teacher representation; DINO matches a student softmax to a centred, sharpened teacher. The job is still a geometry. The anti-collapse device is not.',
      ],
    },
    {
      kicker: 'MECHANICS',
      ...
      body: [
        'If every vector is the same constant, agreement is free. Collapse is the encoder discovering that the cheapest way to make two views agree is to ignore the image. Contrastive methods fight it with negatives, large batches, and a projection head you throw away. SimSiam needs a stop-gradient; BYOL adds a momentum teacher; DINO uses a momentum teacher plus centring and sharpening. Those are why the space is not a point.',
    ...
    },
    {
      ...
      body: [
        ...
        'A frozen DINO encoder on endoscopic video is that geometry spent on a new camera, not a new method. Downstream phase heads and detectors do not have to be distilled. A YOLO trained on potholes is a labelled detector. Mixing those jobs in one sentence is how a supervised paper gets misread as pretraining.',
      ],
    },
  ],
  instance: {
    quote:
      'DINOv2 features are what I actually used on endoscopic video: a frozen encoder plus a head, not a DINO training run, and not SimCLR. The domain question is whether teacher–student features still separate a hook from tissue when the camera is wet and close.',
