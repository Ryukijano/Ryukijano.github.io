export default {
  ...
  taxonomy: {
    ...
    caption:
      'Schematic of three supervision regimes. NVIDIA-style neural pre-decoders that feed matching count as supervised machines in this taxonomy, though nothing here ships their code.',
    branches: [
  ...
  sections: [
    {
      ...
      body: [
        'Supervised decoding trains against a label a fridge will not print. Those labels are not one teacher: Stim’s default sampler yields detectors plus the observable flip, not the Pauli frame; the true fault list is a further leak; imitating MWPM is imitating another decoder that never saw the error. A network can predict an observable, a heat map of likely faults, or local corrections whose residual is handed to MWPM.',
    ...
    },
    {
      ...
      body: [
        'If your training set includes the true error, you are answering how well a model imitates an oracle no hardware gets to be. Still useful: a pre-decoder, a proposal, a capacity study. It is not the same paper as “we learned the channel from s.”',
    ...
    },
    {
      ...
      body: [
        'The workbench can emit supervised labels because a simulator sampled the shot: by default Stim gives detectors and the observable flip, not the Pauli frame. That is a simulator privilege, closer to training a phase head on someone else’s labels than to a fridge. Transfer to a device is a domain shift: leakage, crosstalk, time-varying calibration. A number that does not name its simulator and decoder is not evidence.',
        'If a sweep withholds the true error, the table has to say so; otherwise two columns are not measuring the same thing.',
      ],
    },
  ],
  instance: {
    quote:
      'The workbench can emit supervised labels because a simulator sampled the shot: by default Stim gives detectors and the observable flip, not the Pauli frame. That is a gift. On a device I would not have it. Keeping those two regimes in different columns is what the decoder is allowed to know.',
